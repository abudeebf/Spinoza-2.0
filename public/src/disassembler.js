...
function access_string(access_flags) {
    ...
    return ordered_flags.filter(function (flag) { return access_flags[flag]; })
        .map(function (flag) { return flag + ' '; })
        .join('');
}
...
function make_dis(class_file) {
    ...
    for (sig in methods) {
        ...
        if (!(m.access_flags["native"] || m.access_flags.abstract)) {
            ...
            code.each_opcode(function (idx, oc) { return ops.push({
...
function show_disassembly(dis) {
    ...
    if (dis.annotation_bytes) {
        ...
        var abytes = dis.annotation_bytes.map(function (b) { return pad_left(b.toString(16), 2); }).join(' ');
    ...
    for (var _m = 0, _len4 = dis.methods.length; _m < _len4; _m++) {
        ...
        if (m.code != null) {
            ...
            rv += c.attributes.map(function (attr) { return (typeof attr.disassemblyOutput === "function" ? attr.disassemblyOutput() : void 0) || ''; }).join('');
            if (m.exceptions != null) {
