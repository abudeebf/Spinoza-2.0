...
function activate(pin,psid){
     if (pin== $("#accesscode").val()){
      ...
      $.post(host+'activate',parameters ,function(data){
       location.reload()
   });}
    else{
      alert("You do not have the correct code to access this problemset")
    }
    }
function unactive(user,psid){
  var parameters={psid:psid,user:user}
   $.post(host+'unactive',parameters ,function(data){
       location.reload()
     });
}
...
function viewgraph(){
  ...
  //if (document.getElementById('spt').checked==true)
    //spt=$('#myRange').val();
  if (document.getElementById('syntax').checked==false)
    errors="No"
...
function reloaddebugstat(cid,psid,id){
  var show = $('#show');
    if (show.is(":checked"))
      location.href=host+"debugstat/"+cid+"/"+psid+"/"+id+"/true"
    else
      location.href=host+"debugstat/"+cid+"/"+psid+"/"+id+"/false"  
}
...
function copyToPs(pid,classId,problemset){
 
   var parameters={id1:pid,classId:classId,problemset:problemset};
    $.post(host+'problems/copytops',parameters, function(data) {
      
                      location.href=host+'classes/problems/'+classId+"/"+problemset;
                      });
}
